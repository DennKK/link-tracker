...
public class RateLimitInterceptor implements HandlerInterceptor {
    ...
    @Value("${app.time_duration}")
    private long timeDuration;
    @Value("${app.nano_in_seconds}")
    private long nanoInSeconds;

    private final Map<String, Bucket> cache = new ConcurrentHashMap<>();

    private Bucket newBucket(String apiKey) {
        Refill refill = Refill.intervally(rateLimit, Duration.ofMinutes(timeDuration));
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        if (probe.isConsumed()) {
            ...
            long waitForRefill = probe.getNanosToWaitForRefill() / nanoInSeconds;
